...
Cli.run = function run(processArgv) {
  ...
  // try and run as v2 command first, otherwise call v1 run
  var v2Cli = new V2Cli({
    argv: argv
  });
  ...
  .catch(function(error){
    console.log('run v1 command')
    // call v1
    Cli.oldRun(processArgv, argv);
  });
}

Cli.oldRun = function oldRun(processArgv, argv) {
  try {
...
//TODO
Cli.isV2Command = function isV2Command(argv){
  return true;
}
